"""Historical OfficeQA dual-metric acceptance gate."""
...
class OfficeQAAcceptanceGate:
    ...
    def judge(
        ...
        baseline_em = _metric_value(baseline, "em")
        candidate_em = _metric_value(candidate, "em")
        baseline_f1 = _metric_value(baseline, "f1")
        candidate_f1 = _metric_value(candidate, "f1")
...
def _metric_value(result: MetricResult, name: str) -> float:
    ...
    if (
        ...
        raise ValueError(f"OfficeQA metric {name!r} must be finite")
    return float(value)
...
def _validate_comparable(
    ...
    if baseline.sample_count != candidate.sample_count:
        raise ValueError(
            "OfficeQA baseline and candidate sample counts must match"
        )
    if set(baseline.case_metrics) != set(candidate.case_metrics):
        raise ValueError(
            "OfficeQA baseline and candidate must cover the same cases"
        )
