...
DeepPlanning trains with E2; the formal command matrix evaluates the skills
generated by the new training run.

Before a formal paid run, require successful [strict main-result preflight](reproducibility/main_results.md#generate-the-command-matrix)
with `ready_for_api=true` and no `--allow-missing-secrets` bypass. Use the same
validated configuration for execution. Generic templates and custom dry-runs
do not certify the paper's provider/model/request settings.

Before any paid run, review provider/model, cases, method, scope, split,
