...
ten selected artifacts in total. File identities are recorded in
...
The manifest identifies the skill files and checks their integrity; it does
not report historical evaluation scores. These skills are optional evaluation
inputs, not training initializers or claims of performance. The formal
command matrix instead evaluates the final skills generated by that run.
Both methods start from the same newly generated Stage0 skill.
...
seed 42, exact Stage0 IDs, dataset sizes, model IDs,
...
DeepPlanning uses the E2 optimizer with Momentum, optimization memory, and
Adaptive Edit Budget. Its formal evaluation uses the skill generated by the
corresponding new training run.

All training commands use the frozen configuration and benchmark-specific
prompts. Every prompt used by the paper profile is packaged and indexed in
`skilladam/experiments/prompt_resources.json`; there is no hidden prompt file
needed to execute the main-result command matrix. Optional packaged skills
are never Stage0 or training inputs. Separate ablation, cross-model, and
skill-quality analysis recipes are not part of this matrix.
...
Fill in caller-owned data/runtime paths and keep the formal backend template
selections from the example. Generic templates have different request
defaults; see the [formal backend mapping](main_results.md#generate-the-command-matrix).
Backend configurations contain environment-variable names, never credential
values. Before execution, strict preflight checks the complete main-result
matrix without making provider calls:
...
Require a successful exit and `ready_for_api=true`. The optional
`--allow-missing-secrets` is only for incomplete offline inspection. Neither
command planning nor a lower-level `--dry-run` replaces strict preflight.
Execute only with the same validated backend settings, data, and paths;
repeat preflight after changing them.
...
After runs finish, validate saved metric fields and sample counts, then
summarize the newly generated evaluation outputs:
...
This reads aggregate evaluation metrics. It does not re-score individual
trajectories, establish case-ID uniqueness, or compare with historical result
tables. Inspect the case-level outputs separately when auditing coverage.

Planning, preflight, and verification never call an API. Execution requires
both `--execute` and `--confirm-api-costs` after strict preflight passes.
...
requires both `--execute` and `--confirm-api-costs` after strict preflight
passes and the plan and costs have been reviewed. Generic backend templates
and custom CLI examples are not the formal experiment configuration.
